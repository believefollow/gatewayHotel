...
class GatewayHotelKafkaResourceIT {
    ...
    void setup() {
        ...
        GatewayHotelKafkaResource kafkaResource = new GatewayHotelKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/gateway-hotel-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/gateway-hotel-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
